...
import (
	...
	devxv1alpha1 "github.com/ibm/starter-kit-operator/cmd/swagger/api/model"

	"github.com/gorilla/mux"
)

// RegisterStarterKitRoutes adds skit routes to router
func RegisterStarterKitRoutes(r *mux.Router, p string) {
	rr := r.PathPrefix(p).Subrouter()
	// swagger:operation GET /starterkit repos repoList
	// ---
	// summary: List all starter kits
	// description: Retrieves all installed starter kits
	// parameters:
	// responses:
	//   "200":
	...
	rr.HandleFunc("/apis/devx.ibm.com/v1alpha1/starterkits", GetStarterKits).Methods("GET")
...
type starterkitsResponse struct {
	Code    int                `json:"code"`
	Data    []model.StarterKit `json:"data,omitempty"`
	Message string             `json:"msg,omitempty"`
}

// GetStarterKits returns list of all starter kits
func GetStarterKits(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
...
type starterkitResponse struct {
	Code    int              `json:"code"`
